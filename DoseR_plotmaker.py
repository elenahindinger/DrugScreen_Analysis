...
import numpy as np
import pandas as pd
from DoseR_main_formulas import transform
...
def relative_plotmaker(com1, com2, com3, com4, com5, drugname, sec_analysed,
                       tp_analysed, savename):
    # make copy for safety
    cp1 = com1.copy(deep=True)
    cp2 = com2.copy(deep=True)
    cp3 = com3.copy(deep=True)
    cp4 = com4.copy(deep=True)
    cp5 = com5.copy(deep=True)
    # concatenate all absolute dataframe copies
    df = pd.concat([cp1, cp2, cp3, cp4, cp5]).set_index(np.arange(1, 6))
    # make all 0s NaN
    rel_com = df.replace(to_replace=0, value=np.nan)
    # x axis values
    timepoints = np.arange(1, 6)
    ...
    plt.subplots_adjust(wspace=0.2, hspace=0.2, top=0.87)
    plt.minorticks_off()
    plt.xticks(timepoints, ('0.1', '1', '10', '50', '100'))
    plt.margins(0.1, 0.1)
    ax = [ax1, ax2, ax3, ax4]
    for i in ax:
        i.tick_params(top='off', right='off')
...
def absolute_plotmaker(com1, com2, com3, com4, com5, drugname, sec_analysed,
                       tp_analysed, savename):
    # make copy for safety
    cp1 = com1.copy(deep=True)
    cp2 = com2.copy(deep=True)
...
def plotmaker_means(gr_ctr, het_ctr, gr_com1, gr_com2, gr_com3, gr_com4,
                    gr_com5, het_com1, het_com2, het_com3, het_com4, het_com5,
                    drugname, sec_analysed, tp_analysed, savename):
    # make copy for safety
    ctr = transform(gr_ctr, het_ctr)
    com1 = transform(gr_com1, het_com1)
    com2 = transform(gr_com2, het_com2)
    com3 = transform(gr_com3, het_com3)
    com4 = transform(gr_com4, het_com4)
    com5 = transform(gr_com5, het_com5)
    # x axis values
    timepoints = np.arange(1, 4)

    f, ((ax1, ax2, ax3, ax4, ax5), (ax6, ax7, ax8, ax9, ax10)) = plt.subplots(2, 5, sharex=True,
                                                                             sharey='all', figsize=(11, 6))
    ...
    ax2.bar(timepoints-0.3, com2['gr L mean'], 0.2, yerr=com2['gr L sem'],
            ecolor='k', color='brown', label='gr drug')
    ax2.bar(timepoints-0.1, ctr['gr L mean'], 0.2, yerr=ctr['gr L sem'],
            ecolor='k', color='goldenrod', label='gr ctr')
    ax2.bar(timepoints+0.1, com2['het L mean'], 0.2, yerr=com2['het L sem'],
            ecolor='k', color='k', label='het drug')
    ax2.bar(timepoints+0.3, ctr['het L mean'], 0.2, yerr=ctr['het L sem'],
            ecolor='k', color='grey', label='het ctr')
    ax2.set_title('1 uM', fontsize=10)

    # subplot 3 light, 10uM
    ax3.bar(timepoints-0.3, com3['gr L mean'], 0.2, yerr=com3['gr L sem'],
            ecolor='k', color='brown', label='gr drug')
    ax3.bar(timepoints-0.1, ctr['gr L mean'], 0.2, yerr=ctr['gr L sem'],
            ecolor='k', color='goldenrod', label='gr ctr')
    ax3.bar(timepoints+0.1, com3['het L mean'], 0.2, yerr=com3['het L sem'],
            ecolor='k', color='k', label='het drug')
    ax3.bar(timepoints+0.3, ctr['het L mean'], 0.2, yerr=ctr['het L sem'],
            ecolor='k', color='grey', label='het ctr')
    ax3.set_title('10 uM', fontsize=10)

    # subplot 4 light 50uM
    ax4.bar(timepoints-0.3, com4['gr L mean'], 0.2, yerr=com4['gr L sem'],
            ecolor='k', color='brown', label='gr drug')
    ax4.bar(timepoints-0.1, ctr['gr L mean'], 0.2, yerr=ctr['gr L sem'],
            ecolor='k', color='goldenrod', label='gr ctr')
    ax4.bar(timepoints+0.1, com4['het L mean'], 0.2, yerr=com4['het L sem'],
            ecolor='k', color='k', label='het drug')
    ax4.bar(timepoints+0.3, ctr['het L mean'], 0.2, yerr=ctr['het L sem'],
            ecolor='k', color='grey', label='het ctr')
    ax4.set_title('50 uM', fontsize=10)

    # subplot 5 light 100uM
    ax5.bar(timepoints-0.3, com5['gr L mean'], 0.2, yerr=com5['gr L sem'],
            ecolor='k', color='brown', label='gr drug')
    ax5.bar(timepoints-0.1, ctr['gr L mean'], 0.2, yerr=ctr['gr L sem'],
            ecolor='k', color='goldenrod', label='gr ctr')
    ax5.bar(timepoints+0.1, com5['het L mean'], 0.2, yerr=com5['het L sem'],
            ecolor='k', color='k', label='het drug')
    ax5.bar(timepoints+0.3, ctr['het L mean'], 0.2, yerr=ctr['het L sem'],
            ecolor='k', color='grey', label='het ctr')
    ax5.set_title('100 uM', fontsize=10)
    ax5.legend(loc=5, fontsize='x-small', bbox_to_anchor=(2.2, 0.3),
               borderaxespad=0, title=(sec_analysed + ' sec, tp ' + tp_analysed))

    # subplot 6 light, 0.1uM
    ax6.bar(timepoints-0.3, com1['gr D mean'], 0.2, yerr=com1['gr D sem'],
            ecolor='k', color='brown', label='gr drug')
    ax6.bar(timepoints-0.1, ctr['gr D mean'], 0.2, yerr=ctr['gr D sem'],
            ecolor='k', color='goldenrod', label='gr ctr')
    ax6.bar(timepoints+0.1, com1['het D mean'], 0.2, yerr=com1['het D sem'],
            ecolor='k', color='k', label='het drug')
    ax6.bar(timepoints+0.3, ctr['het D mean'], 0.2, yerr=ctr['het D sem'],
            ecolor='k', color='grey', label='het ctr')
    ax6.set_ylabel('Dark, Distance (mm)', fontsize=10)

    # subplot 7 light, 1uM
    ax7.bar(timepoints-0.3, com2['gr D mean'], 0.2, yerr=com2['gr D sem'],
            ecolor='k', color='brown', label='gr drug')
    ax7.bar(timepoints-0.1, ctr['gr D mean'], 0.2, yerr=ctr['gr D sem'],
            ecolor='k', color='goldenrod', label='gr ctr')
    ax7.bar(timepoints+0.1, com2['het D mean'], 0.2, yerr=com2['het D sem'],
            ecolor='k', color='k', label='het drug')
    ax7.bar(timepoints+0.3, ctr['het D mean'], 0.2, yerr=ctr['het D sem'],
            ecolor='k', color='grey', label='het ctr')

    # subplot 8 light, 10uM
    ax8.bar(timepoints-0.3, com3['gr D mean'], 0.2, yerr=com3['gr D sem'],
            ecolor='k', color='brown', label='gr drug')
    ax8.bar(timepoints-0.1, ctr['gr D mean'], 0.2, yerr=ctr['gr D sem'],
            ecolor='k', color='goldenrod', label='gr ctr')
    ax8.bar(timepoints+0.1, com3['het D mean'], 0.2, yerr=com3['het D sem'],
            ecolor='k', color='k', label='het drug')
    ax8.bar(timepoints+0.3, ctr['het D mean'], 0.2, yerr=ctr['het D sem'],
            ecolor='k', color='grey', label='het ctr')
    ax8.set_xlabel('Incubation Time (h)', fontsize=10)

    # subplot 9 light 50uM
    ax9.bar(timepoints-0.3, com4['gr D mean'], 0.2, yerr=com4['gr D sem'],
            ecolor='k', color='brown', label='gr drug')
    ax9.bar(timepoints-0.1, ctr['gr D mean'], 0.2, yerr=ctr['gr D sem'],
            ecolor='k', color='goldenrod', label='gr ctr')
    ax9.bar(timepoints+0.1, com4['het D mean'], 0.2, yerr=com4['het D sem'],
            ecolor='k', color='k', label='het drug')
    ax9.bar(timepoints+0.3, ctr['het D mean'], 0.2, yerr=ctr['het D sem'],
            ecolor='k', color='grey', label='het ctr')

    # subplot 10 light 100uM
    ax10.bar(timepoints-0.3, com5['gr D mean'], 0.2, yerr=com5['gr D sem'],
             ecolor='k', color='brown', label='gr drug')
    ax10.bar(timepoints-0.1, ctr['gr D mean'], 0.2, yerr=ctr['gr D sem'],
             ecolor='k', color='goldenrod', label='gr ctr')
    ax10.bar(timepoints+0.1, com5['het D mean'], 0.2, yerr=com5['het D sem'],
             ecolor='k', color='k', label='het drug')
    ax10.bar(timepoints+0.3, ctr['het D mean'], 0.2, yerr=ctr['het D sem'],
             ecolor='k', color='grey', label='het ctr')
